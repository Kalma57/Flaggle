...
/**
 * A single real 1v1 "vs Friend" capital-location globe game room, kept alive in memory by
 * {@link CapitalGlobeRoomRegistryBL} for as long as it's active - mirrors
 * {@link com.example.flagdemo.BusinessLayer.GlobeMatchBL.GlobePvpMatchRoom} exactly, just wired
 * to the Capital Globe PvP engines instead of the plain Globe ones. Kept as a fully separate
 * parallel class (rather than generalizing the existing Globe/Flaggle room classes) so none of
 * the working PvP code for those games is ever touched.
 */
public class CapitalGlobePvpMatchRoom implements java.io.Serializable {
    ...
    private volatile PvpCapitalGlobeMatchEngineBL engine;
    /** Created once player 2 joins, only when {@link #gameMode} is BLITZ. */
    private volatile PvpCapitalGlobeBlitzEngineBL blitzEngine;

    private CapitalGlobePvpMatchRoom(String roomCode, PvpGameMode gameMode,
    ...
    public static CapitalGlobePvpMatchRoom createBestOf(String roomCode, int bestOf, String player1Token, CountryController cc) {
        return new CapitalGlobePvpMatchRoom(roomCode, PvpGameMode.BEST_OF_N, bestOf, 0, player1Token, cc);
    }

    /** Creates a new Blitz room (durationSeconds is 60 or 120). */
    public static CapitalGlobePvpMatchRoom createBlitz(String roomCode, int durationSeconds, String player1Token, CountryController cc) {
        return new CapitalGlobePvpMatchRoom(roomCode, PvpGameMode.BLITZ, 0, durationSeconds, player1Token, cc);
    }
    ...
    public synchronized boolean joinPlayer2(String token) {
        ...
        if (gameMode == PvpGameMode.BLITZ) {
            this.blitzEngine = new PvpCapitalGlobeBlitzEngineBL(cc, durationSeconds);
            this.blitzEngine.startMatch();
        } else {
            this.engine = new PvpCapitalGlobeMatchEngineBL(cc, pointsToWin);
    ...
    public PvpCapitalGlobeMatchEngineBL getEngine() { return engine; }
    public PvpCapitalGlobeBlitzEngineBL getBlitzEngine() { return blitzEngine; }
}
